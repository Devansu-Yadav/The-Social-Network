...
const PostCommentCard = ({ post, isBookmarked, isLikedPost }) => {
    return (
        <div className="flex flex-row justify-center mb-4 xs:px-0 xxs:px-0 xs:py-4 xxs:py-3 rounded-none shadow-none border-t border-solid border-gray-200">
            <div className="flex flex-col w-[95%] gap-y-3">
                <div className="flex flex-row items-center justify-between pl-2 pr-2">
                    <Link to={`/profile/${post.userName}`}>
                        <div className="flex flex-row items-center gap-x-2">
                            <span className="inline-flex items-center justify-center shrink-0">
                                <img className="inline-block xs:!w-12 xs:!h-12 xxs:w-10 xxs:h-10 rounded-full" src={`${post.avatar}`} alt={`${post.userName}`} />
                            </span>
                    ...
                    <PostCommentOptionsMenu post={post} />
                </div>
                <div>
