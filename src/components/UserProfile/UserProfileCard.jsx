...
import { isFollowingUser } from "common/helpers";

const UserProfileCard = ({ user, isUserNative, userPosts }) => {
    ...
    const followingUser = isFollowingUser(user, currentLoggedInUserData);
            ...
            <div className="p-3">
                <div className="flex flex-col w-full rounded-2xl shadow-[0_0px_10px_-1px_rgba(0,0,0,0.3)]">
                    ...
                    <div className="flex flex-col items-center">
                        ...
                        <div className={`flex flex-col p-4 w-[95%] relative ${ user.website ? "xs:!bottom-20 xxs:bottom-16": "xxs:bottom-16" }`}>
                            <div className="flex flex-row items-end justify-between">
                                <span className="inline-flex sm:w-32 sm:h-32 xs:!w-28 xs:!h-28 xxs:w-24 xxs:h-24 relative items-center justify-center shrink-0">
                                    <img className="inline-block w-full h-full object-cover max-w-full rounded-full border-4 border-solid border-tertiaryColor" src={`${userAvatar}`} alt={`${user?.userName}`} />
                                </span>
                                <div className="flex flex-col shrink-0 m-0 p-0 items-stretch ml-3 min-w-[77px]">
                                    ...
                                    <button className="outline-none rounded-full text-lg px-6 min-w-[32px] min-h-[32px] bg-primaryColor text-whiteColor hover:bg-primaryColor-dark"
                                        onClick={() => {
                                            followingUser ? unfollowUser(authToken, currentLoggedInUserData, user, user?.id, dispatch)
                            ...
                            <div className="mt-4">
                                ...
                                { user.website && <div className="flex flex-row mt-4 items-center absolute bottom-[-10%]">
                                    <div className="flex flex-row items-center gap-x-2 text-base text-gray-600">
                                        ...
                                        <a className="cursor-pointer text-primaryColor hover:underline" href={user.website} rel="noreferrer" target="_blank">{user?.website}</a>
                                    </div>
                                </div> }
                            </div>
